<template>
  <div
    class="min-h-screen bg-linear-to-br from-gray-900 via-gray-800 to-amber-900/30 pt-32 pb-32"
  >
    <!-- Breadcrumb -->
    <nav
      aria-label="Breadcrumb"
      class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-336 pb-8"
    >
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-336">
      <!-- Page Title -->
      <h1 class="text-3xl sm:text-4xl font-bold text-white text-center mb-16">
        {{ props.pageTitle }}
      </h1>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-9 gap-8 lg:gap-12">
        ...
        <div class="lg:col-span-4 space-y-8">
          <!-- Main Product Image -->
          <div
            class="bg-linear-to-br from-slate-800 via-slate-800 to-slate-900 rounded-2xl p-8 aspect-square flex items-center justify-center shadow-xl ring-1 ring-white/5"
          >
            <img
          ...
          <!-- Thumbnail Gallery -->
          <div
            class="bg-black/20 backdrop-blur-xs rounded-xl p-4 ring-1 ring-white/5"
          >
              ...
              <button
                ...
                :class="
                  selectedImage === index
                    ...
                    : 'ring-gray-700/50 hover:ring-gray-600'
                "
              >
                <img
                  :src="image"
          ...
          <div
            class="bg-black/20 backdrop-blur-xs rounded-xl ring-1 ring-white/5 shadow-lg"
          >
            ...
            <div class="p-6">
              ...
              >
                <p class="text-base">
                  Military challenge coins are a time-honored tradition
                  representing unit pride, achievement, and camaraderie. Our
                  premium challenge coins feature high-quality metal
                  construction with detailed enamel work. Each coin is crafted
                  to honor military service and can be customized with unit
                  insignia, mottos, and commemorative designs. Perfect for
                  recognition ceremonies, unit gifts, and preserving military
                  heritage.
                </p>
              </div>
              <div v-else-if="activeTab === 'Artwork'" class="text-gray-300">
                <p class="text-base">
                  Our design team works with you to create custom artwork that
                  captures your unit's identity and mission. We provide digital
                  proofs and unlimited revisions to ensure your vision comes to
                  life.
                </p>
              </div>
              <div v-else-if="activeTab === 'Turnaround'" class="text-gray-300">
                <p class="text-base">
                  Standard production time is 10-14 business days after artwork
                  approval. Rush orders available for 7-day delivery with
                  additional fees.
                </p>
              </div>
              <div v-else-if="activeTab === 'Shipping'" class="text-gray-300">
                <p class="text-base">
                  Free shipping on orders over $500. Standard shipping via FedEx
                  Ground. Expedited shipping options available.
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="lg:col-span-5 space-y-6">
          ...
          <div
            class="bg-slate-800/80 backdrop-blur-xs rounded-xl p-6 shadow-xl ring-1 ring-white/5"
          >
            <!-- Header with step indicator -->
            <div class="flex items-center gap-4 mb-6">
              <div
                class="flex items-center justify-center w-8 h-8 rounded-full bg-orange-500 text-white font-semibold text-sm shadow-lg"
              >
                1
              </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                ...
                <div class="space-y-2.5">
                  <label v-for="size in sizes" :key="size" class="block">
                    <div
                      class="flex items-center bg-slate-700/50 rounded-lg p-3 cursor-pointer hover:bg-slate-700 transition-colors ring-1 ring-white/5"
                      :class="{
                    ...
                    >
                      ...
                      />
                      <span class="ml-3 text-sm font-medium text-slate-200"
                        >{{ size }}"</span
                      >
                    </div>
                  </label>
              ...
              <div>
                ...
                <div class="space-y-2.5">
                  ...
                  >
                    <div
                      class="flex items-center justify-between bg-slate-700/50 rounded-lg p-3 cursor-pointer hover:bg-slate-700 transition-colors ring-1 ring-white/5"
                      :class="{
                        'ring-2 ring-orange-500 bg-slate-700':
                    ...
                    >
                      <div class="flex items-center">
                        ...
                        />
                        <span class="ml-3 text-sm font-medium text-slate-200">{{
                          formatQuantity(option.amount)
                        }}</span>
                      </div>
                      <div class="text-right flex items-center space-x-3">
                        <span
                          class="text-sm font-medium text-slate-300"
                          v-if="getPricePerUnit(option.amount)"
                          >${{ getPricePerUnit(option.amount) }} per coin</span
                        >
                        <span
                          v-if="option.savings"
                          class="text-xs font-semibold bg-orange-500/10 text-orange-400 px-2 py-0.5 rounded-full"
                        >
                          SAVE {{ option.savings }}%
                        </span>
                      </div>
                ...
                <div class="mt-4 text-right">
                  <button
                    class="text-sm font-medium text-orange-400 hover:text-orange-300 inline-flex items-center"
                  >
                    View bulk pricing
                    <svg
                      class="ml-1 h-4 w-4"
          ...
          >
            ...
            >
              <div class="flex items-center space-x-4">
                <div
                  class="w-8 h-8 bg-slate-700 rounded-full flex items-center justify-center text-white font-semibold text-sm shadow-lg"
                >
                  2
                </div>
            ...
            <div v-if="platingExpanded" class="mt-6">
              <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                >
                  ...
                  />
                  <div
                    class="rounded-xl p-4 bg-slate-700/50 ring-1 ring-white/5 hover:bg-slate-700/70 transition-all duration-200 overflow-hidden peer-checked:ring-2 peer-checked:ring-orange-500 peer-checked:bg-slate-700"
                  >
                    <!-- Image container with adjusted aspect ratio -->
                    <div
                      class="w-full mb-3 rounded-lg bg-slate-800/50 overflow-hidden"
                      style="aspect-ratio: 685/363"
                    >
                      <img
                        :src="plating.image"
                        :alt="plating.name"
                    ...
                    <!-- Label -->
                    <div
                      class="text-sm font-medium text-slate-200 group-hover:text-orange-400 transition-colors text-center"
                    >
                      {{ plating.name }}
                    </div>
                    <!-- Selected indicator -->
                    <div
                      class="absolute top-2 right-2 w-4 h-4 rounded-full bg-orange-500 transform scale-0 peer-checked:scale-100 transition-transform duration-200"
                    ></div>
                  </div>
                </label>
              </div>
            ...
            <div v-if="packagingExpanded" class="mt-6">
              <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                >
                  ...
                  />
                  <div
                    class="aspect-square rounded-xl p-4 bg-slate-700/50 ring-1 ring-white/5 hover:bg-slate-700/70 transition-all duration-200 overflow-hidden peer-checked:ring-2 peer-checked:ring-orange-500 peer-checked:bg-slate-700 flex flex-col items-center"
                  >
                    <!-- Price badge -->
                    <span
                      class="absolute top-2 left-2 bg-slate-100 text-slate-700 text-xs font-semibold px-2 py-0.5 rounded-full shadow"
                    >
                      {{
                        option.price === 0
                          ? "Free"
                    ...
                    <!-- Image container -->
                    <div
                      class="w-full h-24 mb-3 rounded-lg bg-slate-800/50 overflow-hidden flex items-center justify-center"
                    >
                      <img
                        :src="option.image"
                        :alt="option.name"
          ...
          <div ref="pricingSummary" class="sticky bottom-6">
            <div
              class="bg-slate-800/95 backdrop-blur-xs rounded-xl p-6 shadow-xl ring-1 ring-white/5"
            >
              <div class="text-center mb-4">
                <div class="text-slate-400 text-sm font-medium mb-1.5">
                  Estimated Total*
                </div>
                ...
                <div class="text-slate-500 text-xs mt-2">
                  *Final price determined after artwork approval
                </div>
              </div>

              <button
                @click="openContactForm"
                class="w-full bg-linear-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-semibold py-3.5 px-6 rounded-lg transform transition-all duration-300 ease-in-out hover:-translate-y-0.5 flex items-center justify-center space-x-2 shadow-lg mb-6"
              >
                <svg
                  class="h-5 w-5"
                  fill="none"
    ...
    <div
      v-show="false"
      class="mt-24 w-full bg-linear-to-b from-transparent via-slate-900/50 to-slate-900/50"
    >
      <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-336 py-16">
        <div class="text-center mb-16">
          <h2 class="text-3xl sm:text-4xl font-bold text-white mb-4">
            Trusted by Military Units Worldwide
          </h2>
          <p class="text-lg text-slate-400 max-w-2xl mx-auto">
            Join thousands of military units who trust us with their challenge
            coin traditions.
          </p>
        </div>
...
const getPackagingPrice = () => {
  const packaging = packagingOptions.find(
    (p) => p.name === selectedPackaging.value,
  );
