<template>
  ...
  <div
    class="relative transform rounded-2xl bg-slate-800 shadow-2xl transition-all max-w-2xl w-full max-h-[90vh] overflow-hidden"
  >
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-slate-400 mt-1">
              We'll create a custom design proof at no charge
            </p>
          </div>
          <button
            @click="$emit('close')"
      ...
      <form @submit.prevent="submitQuote" class="p-6 space-y-6">
          ...
          <div class="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <span class="text-white ml-2">{{
                formatQuantity(quoteData.quantity)
              }}</span>
            </div>
            <div>
        ...
        <div class="pt-6 border-t border-slate-700">
          <button
            ...
            class="w-full bg-linear-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 disabled:from-slate-600 disabled:to-slate-700 text-white font-semibold py-4 px-6 rounded-lg transition-all duration-300 flex items-center justify-center space-x-2"
          >
            <svg
              v-if="isSubmitting"
          ...
          <p class="text-sm text-slate-400 text-center mt-4">
            By submitting, you agree to receive communications about your quote
            request. We'll respond within 24 hours with your custom design
            proof.
          </p>
        </div>
      </form>
...
const addFiles = (files) => {
  const validFiles = files.filter((file) => {
    ...
    if (!validTypes.includes(file.type)) {
      alert(
        `${file.name} is not a valid file type. Please upload PNG, JPG, or PDF files.`,
      );
      return false;
    }
